...
<html>
	...
	<body>
		...
		if( is_null( $email ) || $email===false ){ $email = filter_input( INPUT_GET, "email", FILTER_SANITIZE_EMAIL ); }
		...
		// template for handling postbacks
			...
			if( $sth = $dice->dbconn->prepare( $sql ) ){
			  ...
			  echo "A DB error has occured, please contact an admin. (2-"; var_dump( $dice->dbconn->errno );
			  echo ")";
			  exit;
			}
