	...
	<body>
		...
		// template for handling postbacks
			...
			if( $sth = $dice->dbconn->prepare( $sql ) ){
				...
				$sth->bind_result($emailColumn);//Read below V
				while($sth->fetch()){
					$rows[] = $emailColumn;//RoiEX (Me) Changed this variable name from $email to $emailColumn
				}//bind_result changed the var to something unreadable wich prevented emails from being sent after registration!
			} else {
				echo "A DB error has occured, please contact an admin. ("; var_dump( $dice->dbconn->errno );
				echo ")";
				exit;
			}
